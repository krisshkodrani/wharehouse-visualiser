...
public class PlacementAdvisor {
  ...
  public PlacementAdvisor(ObjectMapper mapper, WarehouseProperties properties) {
    this.mapper = mapper;
    this.provider = properties.aiProvider();
    this.apiKey = properties.openrouterApiKey();
    this.model = properties.openrouterModel();
    this.openRouterProvider = properties.openrouterProvider().trim();
    SimpleClientHttpRequestFactory requestFactory = new SimpleClientHttpRequestFactory();
    requestFactory.setConnectTimeout(Duration.ofSeconds(Math.min(properties.openrouterTimeoutSeconds(), 15)));
    requestFactory.setReadTimeout(Duration.ofSeconds(properties.openrouterTimeoutSeconds()));
    this.client = RestClient.builder().baseUrl("https://openrouter.ai/api/v1").requestFactory(requestFactory).build();
  }

  public ApiModels.PlacementPlan propose(List<ApiModels.IncomingLoad> loads, List<ApiModels.CandidateSlot> candidates,
